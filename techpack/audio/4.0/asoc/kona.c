...
static void msm_add_tdm_snd_controls(struct snd_soc_codec *codec)
{
	snd_soc_add_codec_controls(codec, msm_tdm_snd_controls,
...
static void msm_add_mi2s_snd_controls(struct snd_soc_codec *codec)
{
	snd_soc_add_codec_controls(codec, msm_mi2s_snd_controls,
...
static void msm_add_auxpcm_snd_controls(struct snd_soc_codec *codec)
{
	snd_soc_add_codec_controls(codec, msm_auxpcm_snd_controls,
...
static int msm_int_audrx_init(struct snd_soc_pcm_runtime *rtd)
{
	...
	msm_add_tdm_snd_controls(codec);
	msm_add_mi2s_snd_controls(codec);
	msm_add_auxpcm_snd_controls(codec);
...
static int msm_aux_codec_init(struct snd_soc_component *component)
{
			...
			if (data != NULL) {
				if (!strncmp(data, "wcd937x",
					...
					wcd937x_info_create_codec_entry(
						pdata->codec_root, codec);
					break;
				} else if (!strncmp( data, "wcd938x",
							sizeof("wcd938x"))) {
					wcd938x_info_create_codec_entry(
						pdata->codec_root, codec);
					break;
				}
			}
	...
	codec_variant = wcd938x_get_codec_variant(codec);
	dev_dbg(codec->dev, "%s: variant %d\n", __func__, codec_variant);
	...
	if (ret < 0) {
		dev_err(codec->dev, "%s: add codec specific snd controls failed: %d\n",
			__func__, ret);
		return ret;
	}
...
mbhc_cfg_cal:
	...
	if (data != NULL) {
		...
		else if (!strncmp( data, "wcd938x", sizeof("wcd938x")))
			ret = wcd938x_mbhc_hs_detect(codec, &wcd_mbhc_cfg);
	}
	if (ret) {
		dev_err(codec->dev, "%s: mbhc hs detect failed, err:%d\n",
			__func__, ret);
		goto err_hs_detect;
	}
	return 0;
...
#ifdef ODM_LQ_EDIT
#ifdef ODM_TARGET_DEVICE_206B1
	sia81xx_num = soc_sia81xx_get_aux_num(pdev);
	card->num_aux_devs += sia81xx_num;
	card->num_configs += sia81xx_num;
#endif
#endif
...
#ifdef ODM_LQ_EDIT
#ifdef ODM_TARGET_DEVICE_206B1
	soc_sia81xx_init(pdev, msm_aux_dev + wsa_max_devs + codec_aux_dev_cnt, sia81xx_num,
		msm_codec_conf + wsa_max_devs + codec_aux_dev_cnt, sia81xx_num);
#endif
#endif
