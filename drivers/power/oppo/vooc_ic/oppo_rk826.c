...
#if 1//def CONFIG_OPPO_CHARGER_MTK
...
static bool rk826_fw_update_check(struct oppo_vooc_chip *chip)
{
	...
	chg_debug("rk826_fw_update_check start\n");
	...
	for (i = chip->fw_data_count - 11; i <= chip->fw_data_count - 4; i++) {
		...
		if (i == chip->fw_data_count - 4){
			...
			chg_debug("fw_mcu_version :0x%x,data_buf:0x%x\n",chip->fw_mcu_version,data_buf[0]);
		}
	...
	if(fw_type_check_result)
		goto fw_update_check_err;
	chg_err("rk826_fw_data check success\n");
...
int Download_00_code(struct oppo_vooc_chip *chip)
{
	...
	chg_debug("size: %d\n", size);
	chg_debug("%s: erase_rk826_00  start\n", __func__);
	do {
	...
	chg_debug("%s: erase_rk826_00 end\n", __func__);
	return 0;
}

int Download_ff_code(struct oppo_vooc_chip *chip)
{
	u8 transfer_buf[TRANSFER_LIMIT];
	u32 onetime_size = TRANSFER_LIMIT - 8;
	u32 index = 0;
	...
	int size=16384;// erase 16kb
	chg_debug("size: %d\n", size);
	chg_debug("%s: erase_rk826_ff start\n", __func__);
	do {
	...
	chg_debug("%s: erase_rk826_ff end\n", __func__);
	return 0;
}
static int rk826_fw_write_00_code(struct oppo_vooc_chip *chip)
{
	int ret = 0;
	int iTryCount = 3;
	struct_req req = {0};
	...
	req.length =  16384;
	req.timeout = 0;
	req.fw_crc = js_hash(chip->firmware_data, chip->fw_data_count);
	...
	// read state firwware
	ret = oppo_i2c_dma_read(chip->client, REG_STATE, 4, read_buf);
	chg_debug("read state firwware: %x\n", *(u32 *)read_buf);
	if (ret < 0) {
...
static int rk826_fw_write_ff_code(struct oppo_vooc_chip *chip)
{
	int ret = 0;
	int iTryCount = 3;
	struct_req req = {0};
...
static int rk826_fw_update(struct oppo_vooc_chip *chip)
{
	...
	msleep(10);
	oppo_i2c_dma_write(chip->client, REG_SYS0, 4, (u8 *)(&force_dis_update_flag));
	msleep(2);
	oppo_i2c_dma_write(chip->client, REG_RESET, 4, (u8 *)(&sw_reset_flag));
...
static int rk826_get_fw_verion_from_ic(struct oppo_vooc_chip *chip)
{
	...
	if (oppo_is_power_off_charging(chip) == true || oppo_is_charger_reboot(chip) == true) {
		...
		update_result = rk826_fw_update(chip);//not used
...
static int rk826_fw_check_then_recover(struct oppo_vooc_chip *chip)
{
	...
	if (oppo_is_power_off_charging(chip) == true || oppo_is_charger_reboot(chip) == true) {
		...
		update_result = rk826_fw_write_00_code(chip);
		update_result = rk826_fw_update(chip);//for factory line smt update
		if (!update_result){
			oppo_i2c_dma_write(chip->client, REG_SYS0, 4, (u8 *)(&force_dis_update_flag));
			msleep(10);
			oppo_i2c_dma_write(chip->client, REG_RESET, 4, (u8 *)(&sw_reset_flag));
			usleep_range(1000000, 1000000);
			rc = oppo_i2c_dma_read(chip->client, 0x52f8, 2, value_buf);
			chg_debug("rk826 read register 0x52f8 rc = %d\n", rc);
			if (rc < 0) {
				chg_debug("rk826 read register 0x52f8 fail, rc = %d\n", rc);
				chg_debug("rk826 fw upgrade check ok.");
			} else {
				chg_debug("read 0x52f8 success 0x%x,0x%x", value_buf[0],value_buf[1]);
				fw_check_err ++;
			}
		} else {
			msleep(30);
...
static int rk826_driver_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if (chip->vooc_fw_type == VOOC_FW_TYPE_RK826_4400_VOOC_FFC_15C) {
		...
		chip->fw_data_count = sizeof(rk826_fw_data_4400_vooc_ffc_15c);
		chip->fw_data_version = rk826_fw_data_4400_vooc_ffc_15c[chip->fw_data_count - 4];
	} else if (chip->vooc_fw_type == VOOC_FW_TYPE_RK826_4450_SVOOC_6500MA) {
	...
	chg_debug("rk826 probe success,fw_type = 0x%x,fw_version = 0x%x,mcu_version = 0x%x\n",
