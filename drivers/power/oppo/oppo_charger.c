...
static void oppo_chg_check_tbatt_normal_status(struct oppo_chg_chip *chip);
...
static void oppo_get_smooth_soc_switch(struct oppo_chg_chip *chip);
static void oppo_chg_smooth_to_soc(struct oppo_chg_chip *chip);
...
int vooc_logo = 0;
bool is_data_err_work = false;
static void oppo_chg_data_error_ac_online(struct work_struct *work)
{
	struct delayed_work *dwork = to_delayed_work(work);
	struct oppo_chg_chip *chip = container_of(dwork, struct oppo_chg_chip, data_error_ac_online_work);

	oppo_vooc_set_data_err_70(false);
	is_data_err_work = false;
		...
		union power_supply_propval *val)
{
	...
	if (chip->charger_exist) {
		if ((chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP)
			...
			chip->ac_online = true;
			//charger_xlog_printk(CHG_LOG_CRTI, "1 chg_exist:%d, ac_online:%d\n",
			//				chip->charger_exist, chip->ac_online);
		} else {
#ifndef ODM_TARGET_DEVICE_206B1
			if (vooc_logo == 1)
			{
				chip->ac_online = true;
				//charger_xlog_printk(CHG_LOG_CRTI, "6 chg_exist:%d, ac_online:%d\n",
				//			chip->charger_exist, chip->ac_online);
			} else {
				chip->ac_online = false;
				//charger_xlog_printk(CHG_LOG_CRTI, "2 chg_exist:%d, ac_online:%d\n",
				//			chip->charger_exist, chip->ac_online);
			}
#else
			chip->ac_online = false;
		...
		if ((oppo_vooc_get_fastchg_started() == true)
			...
			{
				chip->ac_online = true;
				//charger_xlog_printk(CHG_LOG_CRTI, "4 chg_exist:%d, ac_online:%d\n",
				//			chip->charger_exist, chip->ac_online);
			} else {
				chip->ac_online = false;
				//charger_xlog_printk(CHG_LOG_CRTI, "5 chg_exist:%d, ac_online:%d\n",
				//			chip->charger_exist, chip->ac_online);
			}
#else
			chip->ac_online = false;
		...
		const union power_supply_propval *val)
{
	...
	switch (psp) {
		...
		case POWER_SUPPLY_PROP_CALL_MODE:
			...
			charger_xlog_printk(CHG_LOG_CRTI, "calling_on = %d\n", chip->calling_on);
			break;
#endif
#ifdef CONFIG_OPPO_SHORT_C_BATT_CHECK
#ifdef CONFIG_OPPO_SHORT_USERSPACE
		...
		union power_supply_propval *val)
{
	...
	switch (psp) {
		case POWER_SUPPLY_PROP_STATUS:
			if (oppo_chg_show_vooc_logo_ornot() == 1) {
				if(chip->new_ui_warning_support 
					&& ((chip->tbatt_status == BATTERY_STATUS__WARM_TEMP||chip->tbatt_status == BATTERY_STATUS__COLD_TEMP) && chip->batt_full))
		...
		case POWER_SUPPLY_PROP_CURRENT_NOW:
			if ((!chip->authenticate) && oppo_is_rf_ftm_mode()) {
				chip->icharging = 999;
			} else {
				if (oppo_vooc_get_fastchg_started() == true) {
		...
		const char __user *buff, size_t count, loff_t *ppos)
{
	...
	if (strncmp(proc_chg_cycle_data, "en808", 5) == 0) {
		...
		if (g_charger_chip->dual_charger_support) {
			...
			oppo_chg_set_charging_current(g_charger_chip);
		}
		...
		if (g_charger_chip->chg_ops->oppo_chg_wdt_enable)
			g_charger_chip->chg_ops->oppo_chg_wdt_enable(false);
...
#ifdef VENDOR_EDIT
#define OPPO_CHARGE_LOG_BUF_SIZE 256
static int get_utc_time(char *buf, int max_size)
{
    struct timespec ts;
    struct rtc_time tm;
...
void oppo_chg_shedule_show_bat_info(void){

    if (!g_charger_chip) {
            chg_err(" g_charger_chip NULL,return\n");
            return;
    }
    if(!g_charger_chip->chging_on && !g_charger_chip->led_on){
        schedule_delayed_work(&g_charger_chip->battery_info_show_work, 0);
    }
}
#ifdef ODM_TARGET_DEVICE_206B1
extern int da9313_get_work_mode(void);
#endif
static void oppo_chg_show_battery_info(struct work_struct *work){

    struct delayed_work *dwork = to_delayed_work(work);
    struct oppo_chg_chip *chip = container_of(dwork, struct oppo_chg_chip, battery_info_show_work);
...
#ifdef ODM_TARGET_DEVICE_206B1
        count += snprintf(log_buf+count, OPPO_CHARGE_LOG_BUF_SIZE-count, "RM_POWER_BAT[vol %d, cur %d, temp %d, soc %d, ui_soc %d, fcc %d, rm %d, da %d]",
            chip->batt_volt, chip->icharging, chip->temperature, chip->soc, chip->ui_soc, chip->batt_fcc, chip->batt_rm, da9313_get_work_mode());
#else
        count += snprintf(log_buf+count, OPPO_CHARGE_LOG_BUF_SIZE-count, "RM_POWER_BAT[vol %d, cur %d, temp %d, soc %d, ui_soc %d, fcc %d, rm %d]",
            chip->batt_volt, chip->icharging, chip->temperature, chip->soc, chip->ui_soc, chip->batt_fcc, chip->batt_rm);
#endif
        count += get_utc_time(log_buf+count, OPPO_CHARGE_LOG_BUF_SIZE-count);
        log_buf[count] = '\0';
        printk(KERN_INFO"%s\n", log_buf);
    }
}
...
int oppo_chg_init(struct oppo_chg_chip *chip)
{
	...
	oppo_get_smooth_soc_switch(chip);
	oppo_chg_get_battery_data(chip);
...
//yangmingjin@BSP.POWER.Basic 2019/05/30 add for RM_TAG_POWER_DEBUG
#ifdef VENDOR_EDIT
	INIT_DELAYED_WORK(&chip->battery_info_show_work, oppo_chg_show_battery_info);
#endif
/*VENDOR_EDIT*/
#ifndef ODM_TARGET_DEVICE_206B1
	INIT_DELAYED_WORK(&chip->data_error_ac_online_work, oppo_chg_data_error_ac_online);
#endif
...
int oppo_chg_parse_charger_dt(struct oppo_chg_chip *chip)
{
...
#ifdef OPPO_OPEN_ADBROOT_VERSION
	charger_xlog_printk(CHG_LOG_CRTI,
		"normal_phase1_bat_decidegc = %d, temp_normal_phase1_vfloat_mv = %d, temp_normal_phase1_fastchg_current_ma = %d\n"
	"		normal_phase2_bat_decidegc = %d, temp_normal_phase2_vfloat_mv = %d, temp_normal_phase2_fastchg_current_ma_high = %d\n"
	"		temp_normal_phase2_fastchg_current_ma_low = %d, normal_phase3_bat_decidegc = %d, temp_normal_phase3_vfloat_mv = %d\n"
	"		temp_normal_phase3_fastchg_current_ma_high = %d, temp_normal_phase3_fastchg_current_ma_low = %d, normal_phase4_bat_decidegc = %d\n"
	"		temp_normal_phase4_vfloat_mv = %d, temp_normal_phase4_fastchg_current_ma_high = %d, temp_normal_phase4_fastchg_current_ma_low = %d\n"
	"		normal_phase5_bat_decidegc = %d, temp_normal_phase5_vfloat_mv = %d, temp_normal_phase5_fastchg_current_ma = %d\n"
	"		normal_phase6_bat_decidegc = %d, temp_normal_phase6_vfloat_mv = %d, temp_normal_phase6_fastchg_current_ma = %d\n",
			chip->limits.normal_phase1_bat_decidegc,chip->limits.temp_normal_phase1_vfloat_mv,chip->limits.temp_normal_phase1_fastchg_current_ma,
			chip->limits.normal_phase2_bat_decidegc,chip->limits.temp_normal_phase2_vfloat_mv,chip->limits.temp_normal_phase2_fastchg_current_ma_high,
			chip->limits.temp_normal_phase2_fastchg_current_ma_low,chip->limits.normal_phase3_bat_decidegc,chip->limits.temp_normal_phase3_vfloat_mv,
			chip->limits.temp_normal_phase3_fastchg_current_ma_high,chip->limits.temp_normal_phase3_fastchg_current_ma_low,chip->limits.normal_phase4_bat_decidegc,
			chip->limits.temp_normal_phase4_vfloat_mv,chip->limits.temp_normal_phase4_fastchg_current_ma_high,chip->limits.temp_normal_phase4_fastchg_current_ma_low,
			chip->limits.normal_phase5_bat_decidegc,chip->limits.temp_normal_phase5_vfloat_mv,chip->limits.temp_normal_phase5_fastchg_current_ma,
			chip->limits.normal_phase6_bat_decidegc,chip->limits.temp_normal_phase6_vfloat_mv,chip->limits.temp_normal_phase6_fastchg_current_ma);
#endif
	/*45~55 C*/
...
#ifdef OPPO_OPEN_ADBROOT_VERSION
	charger_xlog_printk(CHG_LOG_CRTI,
			"vbatt_power_off = %d, vbatt_soc_1 = %d, normal_vterm_hw_inc = %d,"
			" non_normal_vterm_hw_inc = %d, vbatt_pdqc_to_5v_thr = %d, vbatt_pdqc_to_9v_thr = %d\n",
			chip->vbatt_power_off,chip->vbatt_soc_1,chip->limits.normal_vterm_hw_inc,
			chip->limits.non_normal_vterm_hw_inc,chip->limits.vbatt_pdqc_to_5v_thr,
			chip->limits.vbatt_pdqc_to_9v_thr);
#endif
...
#ifdef OPPO_OPEN_ADBROOT_VERSION
	charger_xlog_printk(CHG_LOG_CRTI,
			"ff1_normal_fastchg_ma = %d, ffc2_temp_warm_decidegc = %d, ffc2_temp_high_decidegc = %d, ffc2_normal_fastchg_ma = %d\n"
	"		chip->limits.ffc2_warm_fastchg_ma = %d, ffc2_exit_step_ma = %d, ffc_normal_vfloat_sw_limit = %d, ffc2_normal_vfloat_sw_limit = %d\n"
	"		ffc_temp_normal_vfloat_mv = %d, ffc1_temp_normal_vfloat_mv = %d, ffc2_temp_normal_vfloat_mv = %d, ffc_normal_vfloat_over_sw_limit = %d\n"
	"		ffc2_temp_low_decidegc = %d, limits.ff1_exit_step_ma = %d, limits.ff1_warm_exit_step_ma = %d, pd_input_current_charger_ma = %d,"
			" qc_input_current_charger_ma = %d\n",
			chip->limits.ff1_normal_fastchg_ma,chip->limits.ffc2_temp_warm_decidegc,chip->limits.ffc2_temp_high_decidegc,chip->limits.ffc2_normal_fastchg_ma,
			chip->limits.ffc2_warm_fastchg_ma,chip->limits.ffc2_exit_step_ma,chip->limits.ffc1_normal_vfloat_sw_limit,chip->limits.ffc2_normal_vfloat_sw_limit,
			chip->limits.ffc_temp_normal_vfloat_mv,chip->limits.ffc1_temp_normal_vfloat_mv,chip->limits.ffc2_temp_normal_vfloat_mv,
			chip->limits.ffc_normal_vfloat_over_sw_limit,
			chip->limits.ffc2_temp_low_decidegc,chip->limits.ff1_exit_step_ma,chip->limits.ff1_warm_exit_step_ma,chip->limits.pd_input_current_charger_ma,
			chip->limits.qc_input_current_charger_ma);
#endif
...
#ifdef OPPO_OPEN_ADBROOT_VERSION
	charger_xlog_printk(CHG_LOG_CRTI,
			"default_iterm_ma = %d, default_temp_normal_fastchg_current_ma = %d, default_normal_vfloat_sw_limit = %d,"
			" default_temp_normal_vfloat_mv = %d, default_normal_vfloat_over_sw_limit = %d\n",
			chip->limits.default_iterm_ma,chip->limits.default_temp_normal_fastchg_current_ma,chip->limits.default_normal_vfloat_sw_limit,
...
#ifdef OPPO_OPEN_ADBROOT_VERSION
	charger_xlog_printk(CHG_LOG_CRTI,
			"default_temp_little_cool_fastchg_current_ma = %d, default_little_cool_vfloat_sw_limit = %d, default_temp_little_cool_vfloat_mv = %d,"
			" default_little_cool_vfloat_over_sw_limit = %d\n",
			chip->limits.default_temp_little_cool_fastchg_current_ma,chip->limits.default_little_cool_vfloat_sw_limit,chip->limits.default_temp_little_cool_vfloat_mv,
			chip->limits.default_little_cool_vfloat_over_sw_limit);
#endif
...
#ifdef OPPO_OPEN_ADBROOT_VERSION
	charger_xlog_printk(CHG_LOG_CRTI,
			"input_current_charger_ma = %d, input_current_usb_ma = %d, input_current_led_ma = %d, input_current_led_ma_normal = %d\n"
	"		input_current_led_ma_warm = %d, input_current_led_ma_high = %d, temp_normal_fastchg_current_ma = %d, temp_normal_vfloat_mv = %d\n"
	...
	"		temp_cool_vfloat_mv = %d, temp_cool_fastchg_current_ma_high = %d, temp_cool_fastchg_current_ma_low = %d, little_cool_bat_decidegc = %d\n"
	"		temp_little_cool_vfloat_mv = %d, temp_little_cool_fastchg_current_ma = %d, normal_bat_decidegc = %d, warm_bat_decidegc = %d\n"
	"		temp_warm_vfloat_mv = %d, temp_warm_fastchg_current_ma = %d, hot_bat_decidegc = %d, non_standard_vfloat_mv = %d\n"
	"		non_standard_fastchg_current_ma = %d, max_chg_time_sec = %d, charger_hv_thr = %d, charger_lv_thr = %d, vbatt_full_thr = %d\n"
	"		vbatt_hv_thr = %d, vfloat_step_mv = %d, vooc_project = %d, suspend_after_full = %d, ext_gauge = %d, sw_vfloat_enable = %d\n"
	"		temp_little_cold_fastchg_current_ma_low = %d, temp_little_cold_fastchg_current_ma_high = %d, charger_current_vooc_ma_normal = %d\n"
	"		chip->ffc_support = %d, chip->dual_ffc = %d, chip->new_ui_warning_support = %d, chip->smooth_switch = %d\n",
			chip->limits.input_current_charger_ma,chip->limits.input_current_usb_ma,chip->limits.input_current_led_ma,chip->limits.input_current_led_ma_normal,
			chip->limits.input_current_led_ma_warm,chip->limits.input_current_led_ma_high,chip->limits.temp_normal_fastchg_current_ma,chip->limits.temp_normal_vfloat_mv,
			chip->limits.iterm_ma,chip->limits.recharge_mv,chip->limits.cold_bat_decidegc,chip->limits.temp_cold_vfloat_mv,chip->limits.temp_cold_fastchg_current_ma,
			chip->limits.little_cold_bat_decidegc,chip->limits.temp_little_cold_vfloat_mv,chip->limits.temp_little_cold_fastchg_current_ma,chip->limits.cool_bat_decidegc,
			chip->limits.temp_cool_vfloat_mv,chip->limits.temp_cool_fastchg_current_ma_high,chip->limits.temp_cool_fastchg_current_ma_low,chip->limits.little_cool_bat_decidegc,
			chip->limits.temp_little_cool_vfloat_mv,chip->limits.temp_little_cool_fastchg_current_ma,chip->limits.normal_bat_decidegc,chip->limits.warm_bat_decidegc,
			chip->limits.temp_warm_vfloat_mv,chip->limits.temp_warm_fastchg_current_ma,chip->limits.hot_bat_decidegc,chip->limits.non_standard_vfloat_mv,
			...
			chip->limits.vbatt_hv_thr,chip->limits.vfloat_step_mv,chip->vooc_project,chip->suspend_after_full,chip->external_gauge,chip->limits.sw_vfloat_over_protect_enable,
			chip->limits.temp_little_cold_fastchg_current_ma_low,chip->limits.temp_little_cold_fastchg_current_ma_high,chip->limits.charger_current_vooc_ma_normal,
...
int oppo_chg_get_tbatt_normal_charging_current(struct oppo_chg_chip *chip)
{
...
static void oppo_chg_set_charging_current(struct oppo_chg_chip *chip)
{
	...
	switch (chip->tbatt_status) {
		...
		case BATTERY_STATUS__NORMAL:
			if (chip->dual_charger_support) {
				charging_current = oppo_chg_get_tbatt_normal_charging_current(chip);
			}
			else
	...
	//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max],set charging_current = %d\n",charging_current);
...
static int oppo_chg_get_float_voltage(struct oppo_chg_chip *chip)
{
	...
	//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max],get float voltage = %d\n",flv);
	return flv;
}
...
static void oppo_chg_set_float_voltage(struct oppo_chg_chip *chip)
{
	...
	//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max], set float voltage = %d\n",flv);
...
static void oppo_chg_vfloat_over_check(struct oppo_chg_chip *chip)
{
	...
	if (oppo_vooc_get_fastchg_ing() == true) {
		...
		if(!(chip->total_time%60)){
			...
			//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max],vfloat_sw_set = %d\n",chip->limits.vfloat_sw_set);
		}
		#endif /* VENDOR_EDIT */
#endif
...
static void battery_temp_anti_shake_handle(struct oppo_chg_chip *chip)
{
	...
	charger_xlog_printk(CHG_LOG_CRTI, "cold_bat_decidegc = %d, little_cold_bat_decidegc = %d, cool_bat_decidegc = %d, \
		little_cool_bat_decidegc = %d, normal_bat_decidegc = %d, warm_bat_decidegc = %d, \
		hot_bat_decidegc = %d, shake = %d - %d\n",
		chip->limits.cold_bat_decidegc, chip->limits.little_cold_bat_decidegc,
		chip->limits.little_cool_bat_decidegc, chip->limits.little_cool_bat_decidegc,
		chip->limits.normal_bat_decidegc,
		chip->limits.warm_bat_decidegc,
		chip->limits.hot_bat_decidegc,
...
static void battery_temp_normal_anti_shake_handle(struct oppo_chg_chip *chip)
{
...
{
	int batt_temp = chip->temperature;
	OPPO_CHG_TBATT_NORMAL_STATUS tbatt_normal_status = chip->tbatt_normal_status;
...
static void oppo_chg_variables_init(struct oppo_chg_chip *chip)
{
	...
	chip->usb_status = 0;
	init_waitqueue_head( &chip->oppo_usbtemp_wq);
#endif
...
static void oppo_chg_check_rechg_status(struct oppo_chg_chip *chip)
{
	...
	//charger_xlog_printk(CHG_LOG_CRTI,
	//		"[oppo_chg_max],Battery rechg! batt_volt = %d, recharging_vol = %d,rechging_cnt = %d\n",
	//		nbat_vol, recharging_vol,rechging_cnt);
...
static void oppo_chg_get_battery_data(struct oppo_chg_chip *chip)
{
	...
	static int read_soc = 0;
	if ((!chip->authenticate) && oppo_is_rf_ftm_mode()) {
		charger_xlog_printk(CHG_LOG_CRTI, "[Battery]oppo_chg_get_battery_data,ftm mode \n");
		chip->batt_volt = 3800;
		chip->batt_volt_max = 3800;
		chip->batt_volt_min = 3800;
		chip->icharging = 999;
		chip->temperature = -400;
		chip->soc = 50;
		chip->batt_fcc = 1;
		chip->batt_cc = 1;
		chip->batt_soh = 1;
...
/*Hongbin.Chen@ODM_LQ.BSP.CHG,	2020/08/21 ,Modify after soc 0 poweron soc display 100  start*/
	if (chip->soc == 100 && chip->batt_volt < 3500) {
		read_soc = oppo_gauge_get_batt_soc();
		chip->soc = read_soc;
		chg_err("read_soc = %d, chip->soc = %d\n", read_soc, chip->soc);
	}
/*Hongbin.Chen@ODM_LQ.BSP.CHG,	2020/08/21 ,Modify after soc 0 poweron soc display 100  end*/
	...
	if (chip->chg_ops->get_ibus && oppo_vooc_get_allow_reading() == true){
		chip->ibus = chip->chg_ops->get_ibus();
	} else {
		chip->ibus = -1;
	}
	if(chip->smooth_switch)
		oppo_chg_smooth_to_soc(chip);
	else
		chip->smooth_soc = chip->soc;
		
	if(!chip->healthd_ready && chip->smooth_switch == 1) {
		chg_err("tongfeng test gauge soc[%d] \n", chip->soc);
		chip->soc = -1;
	}
	if (ui_soc_cp_flag == 0) {
	...
	if(status == DISCHARGING_STATUS){
		if(chip->soc >= 96 && chip->soc <= 100 && chip->ui_soc == 100){
				...
				if(chip->smooth_soc > chip->soc){
					...
					if((capacity - chip->batt_rm) >= smooth_diff || (chip->smooth_soc - chip->soc >5)){
							chip->smooth_soc--;
							capacity = chip->batt_rm;
							smooth_diff = capacity/chip->smooth_soc;
					}
					...
					pr_err("[oppo_chg_smooth_to_soc] smooth_soc[%d],capacity[%d],smooth_diff[%d]",chip->smooth_soc,capacity,smooth_diff);
				} else {
					chip->smooth_soc =chip->soc;
				}
			}
		...
		if(chip->smooth_soc > chip->soc){
			...
			pr_err("[oppo_chg_smooth_to_soc]charging smooth_soc[%d],capacity[%d],smooth_diff[%d],soc_pre[%d]",chip->smooth_soc,capacity,smooth_diff,soc_pre);
		} else {
			chip->smooth_soc = chip->soc;
		}
	}
...
static void oppo_chg_update_ui_soc(struct oppo_chg_chip *chip)
{
	...
	if (chip->charger_exist && chip->batt_exist
		...
		if (oppo_short_c_batt_is_prohibit_chg(chip)) {
			...
			if(chip->new_ui_warning_support && (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP || chip->tbatt_status == BATTERY_STATUS__COLD_TEMP)){
				chip->prop_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
			}else{
				chip->prop_status = POWER_SUPPLY_STATUS_FULL;
			}
		...
		if (chip->ui_soc == 100 
				...
				|| oppo_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC)) {
			if (++cnt >= 12) {
				charger_xlog_printk(CHG_LOG_CRTI, "cnt:%d\n",cnt);
				//cnt = 0;
				chip->batt_full = true;
				chip->total_time = 0;
				chip->in_rechging = false;
		...
		if(chip->smooth_switch && soc_down_limit >= SOC_SYNC_DOWN_RATE_60S){
			soc_down_limit = SOC_SYNC_DOWN_RATE_60S;
		}
		cnt = 0;
		chip->prop_status = POWER_SUPPLY_STATUS_NOT_CHARGING;
		...
		if (chip->ui_soc != ui_soc_pre) {
			chg_debug("full [soc ui_soc smooth_soc down_limit] = [%d %d %d %d]\n",
...
static void oppo_chg_fast_switch_check(struct oppo_chg_chip *chip)
{
	...
	if (chip->charger_type == POWER_SUPPLY_TYPE_USB_DCP) {
		...
		if(oppo_vooc_get_fw_update_status())
		{
			reset_mcu_delay = 0;
		}
		if (!oppo_vooc_get_fastchg_started()
...
static int oppo_chg_check_sw_full(struct oppo_chg_chip *chip)
{
	...
	//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max],vbatt_full_vol_sw = %d,iterm_ma= %d\n",vbatt_full_vol_sw,chip->limits.iterm_ma);
...
static int oppo_chg_check_hw_full(struct oppo_chg_chip *chip)
{
	...
	//charger_xlog_printk(CHG_LOG_CRTI,"[oppo_chg_max],vbatt_full_vol_hw = %d\n",vbatt_full_vol_hw);
	return ret_hw;
}
...
static void oppo_chg_check_status_full(struct oppo_chg_chip *chip)
{
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL)
		...
		if ((chip->recharge_after_full == true || chip->vbatt_num == 2)
			...
			if (chip->dod0_counts <= DOD0_COUNTS+1)
				chip->dod0_counts ++;
...
static void oppo_chg_print_log(struct oppo_chg_chip *chip)
{
	...
	if(0){
		...
		count += snprintf(log_buf+count, OPPO_CHARGE_LOG_BUF_SIZE-count, "CHGR[%d,%d,%d,%d,%d],BAT[%d,%d,%d,%d,%d,%d],GAUGE[%d,%d,%d,%d,%d,%d,%d,%d,%d],"
				"STATUS[0x%x,%d,%d,%d,%d,0x%x],OTHER[%d,%d,%d,%d,%d,%d,%d,%d,%d,%d]\n",
		...
		chip->otg_switch, chip->otg_online, chip->mmi_chg, chip->boot_reason, chip->chargerid_volt, chip->chargerid_volt_got,
		chip->usb_temp_l, chip->usb_temp_r, chip->cool_down, chip->led_on);
...
static void oppo_chg_pd_config(struct oppo_chg_chip *chip)
{
	...
	if (chip->pd_chging == false && chip->chg_ops->oppo_chg_get_pd_type() == true && oppo_chg_show_vooc_logo_ornot() == false) {
...
static void oppo_chg_pdqc_to_normal(struct oppo_chg_chip *chip)
{
	...
	if(oppo_chg_show_vooc_logo_ornot() == true){
		return;
	}
	if (chip->charger_volt > 7500) {
		pdqc_9v = true;
...
static void oppo_chg_dual_charger_config(struct oppo_chg_chip *chip)
{
...
/*Hongbin.Chen@ODM_LQ.BSP.CHG,	2020/08/19 ,Modify precharge voltage for pd/qc current little*/
	...
	/*Hongbin.Chen@ODM_LQ.BSP.CHG,	2020/08/19 ,Modify precharge voltage for pd/qc current little end*/
		...
		oppo_chg_check_tbatt_normal_status(chip);
	...
	oppo_chg_dual_charger_config(chip);
	oppo_chg_pd_config(chip);
	oppo_chg_qc_config(chip);
...
void oppo_chg_soc_update_when_resume(unsigned long sleep_tm_sec)
{
	...
	new_soc = oppo_gauge_get_batt_soc();
	...
	if(g_charger_chip->smooth_switch){
		oppo_chg_smooth_to_soc(g_charger_chip);
	}
...
int oppo_chg_show_vooc_logo_ornot(void)
{
	...
	if (g_charger_chip->chg_ctrl_by_vooc) {
		if (oppo_vooc_get_fastchg_started() == true
				&& oppo_vooc_get_fast_chg_type() == CHARGER_SUBTYPE_FASTCHG_VOOC) {
			charger_xlog_printk(CHG_LOG_FULL,"show_vooc_logo_ornot by vooc\n");
			if (g_charger_chip->prop_status != POWER_SUPPLY_STATUS_FULL
					&& (g_charger_chip->stop_voter == CHG_STOP_VOTER__FULL
					|| g_charger_chip->stop_voter == CHG_STOP_VOTER_NONE)) {
				//chg_err(":1\n");
	...
	if (oppo_vooc_get_fastchg_started()) {
		...
		if ((vooc_logo == 1) && (oppo_vooc_get_data_err_70() == true))
		{
			if (is_data_err_work == false) {
				chg_err(":data_error_ac_online_work\n");
				is_data_err_work = true;
				schedule_delayed_work(&g_charger_chip->data_error_ac_online_work, round_jiffies_relative(msecs_to_jiffies(4500)));
			}
		} else {
			//chg_err(":00\n");
			vooc_logo = 0;
...
bool oppo_get_vbatt_higherthan_xmv(void)
{
        if (!g_charger_chip) {
                return false;
        } else {
...
#ifdef ODM_LQ_EDIT
bool oppo_chg_get_call_mode_status(void)
{
    if (!g_charger_chip) {
        return 0;
    } else {
        return g_charger_chip->calling_on;
    }
}
bool oppo_chg_get_led_mode_status(void)
{
    if (!g_charger_chip) {
        return 0;
    } else {
