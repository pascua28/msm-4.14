...
#ifdef ODM_LQ_EDIT
/*yangcaoping@ODM_LQ@BSP.fingerprint,2019/11/21 for fingerprint*/
#define INVALID_FP_ID_VALUE    0xFF
#define FP_PMIC_GPIO_ID_NODE   "qcom,platform-fp-id-gpio"
#endif /*ODM_LQ_EDIT*/
...
/*yangcaoping@ODM_LQ@BSP.fingerprint,2020/05/01 for fingerprint*/
#if 0
...
static int get_manufacture_id_value(struct fp_data *fp_data)
{
    ...
    uint32_t fp_id_value = INVALID_FP_ID_VALUE;
    ...
    fp_id_value = gpio_get_value(fp_id_gpio);
    dev_info(fp_data->dev, "fp_id_value%d\n", fp_id_value);
...
#ifdef ODM_TARGET_DEVICE_206B1
/*yangcaoping@ODM_LQ@BSP.fingerprint,2020/05/01 for fingerprint*/
    fp_data->fp_id[0] = 1; // just one fp manufacture for M590
    dev_info(fp_data->dev, "fp_id_value%d\n", fp_data->fp_id[0]);
#else
    ret = get_manufacture_id_value(fp_data);
    if (ret) {
        dev_info(fp_data->dev, "failed to get_manufacture_id_value\n");
        ret = -FP_ERROR_GENERAL;
        goto exit;
    }
#endif /*ODM_TARGET_DEVICE_206B1*/
...
int opticalfp_irq_handler(struct fp_underscreen_info* tp_info)
{
    ...
    if (FP_GOODIX_OPTICAL_G3 == fpsensor_type) {
        return gf_opticalfp_irq_handler(tp_info);
    }
    else {
        dev_err(fp_data_ptr->dev, "opticalfp_irq_handler FP_UNKNOWN\n");
        return FP_UNKNOWN;
    }
}
#endif /*ODM_TARGET_DEVICE_206B1*/
...
#ifdef ODM_LQ_EDIT
/*yangcaoping@ODM_LQ@BSP.fingerprint,2019/11/21 for fingerprint*/
module_init(oppo_fp_common_init);
#else
